...
The bottom line is that what @ruby_@ is attached to are either what to support
@ruby@ command or the internal variables and  what @rb_@ is attached to are the
official interfaces of @ruby@ interpretor.
...
what have difficulty most is probably around the development environment of Ruby.
...
interpretors to one. "it's impossible to initialize completely" appears to be
its reason. For example, "the loaded extension libraries could not be removed"
...
What is important is only the last @if@ statement.
