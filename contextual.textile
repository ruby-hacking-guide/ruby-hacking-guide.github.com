h2(#chapter-11-finite-state-scanner). Chapter 11 Finite-state scanner
...
h4(#practical-examples). Practical examples
...
bc(ruby). `cvs diff parse.y`          # command call string
...
h4(#understanding-the-finite-state-scanner). Understanding the finite-state scanner
...
h5(#the-static-approach). The static approach
...
!/images/rhg-c11/ch_contextual_transittobeg.jpg(Transition to @EXPR_BEG@)!
Transition to @EXPR_BEG@
...
h5(#the-dynamic-approach). The dynamic approach
...
h4(#description-of-states). Description of states
...
Comes after the dot in a method call. Handled similarly to @EXPR_FNAME@ Various reserved words are treated as simple identifiers. In an independent position <code>'`'</code> is the name.
...
h3(#line-break-handling). Line-break handling

h4(#the-problem). The problem
...
h3(#reserved-words-and-identical-method-names). Reserved words and identical method names

h4(#the-problem-1). The problem
...
h4(#method-definition). Method definition
...
bc(ruby). call if cond then a else b end
...
h4(#implementation-1). Implementation
...
h3(#the-do-conflict). The @do@ conflict

h4(#the-problem-3). The problem
...
bc(text). /* do conflict experiment */
...
h4(#rule-level-solution). Rule-level solution
...
h4(#symbol-level-solution). Symbol-level solution
...
!/images/rhg-c11/ch_contextual_condp.jpg(Changes of @COND_P()@)!
Changes of @COND_P()@

Using this trick should also make situations like the one shown below easy to deal with.

bc(ruby). while (m do .... end)   # do is an iterator do(kDO)
  ....
end
...
h3(#tlparen_arg1). @tLPAREN_ARG@(1)

h4(#the-problem-4). The problem
...
bc(text). ~/src/ruby % cvs diff -r1.99 -r1.100 parse.y


h4(#investigation). Investigation
...
h4(#the-case-of-one-parameter). The case of one parameter
...
bc(ruby). m(              # Parenthesis not preceded by a space. Method parenthesis ('(')
...
!/images/rhg-c11/ch_contextual_trees.jpg(@method_call@ and @command_call@)!
@method_call@ and @command_call@

h4(#the-case-of-two-parameters-and-more). The case of two parameters and more
...
bc(text). command_args : tLPAREN_ARG args ')'
...
bc(text). command_args : tLPAREN_ARG arg ')'
             | tLPAREN_ARG arg ')'
...
bc(text). command_args    : call_args
...
h3(#tlparen_arg2). @tLPAREN_ARG@(2)

h4(#the-problem-5). The problem
...
h4(#rule-level-solution-1). Rule-level solution
...
bc(ruby). % rubylex-analyser -e 'm (a) { nil }'
+EXPR_BEG
...
The problem here are parts of single lines that come out as @+EXPR_ENDARG@. This indicates a transition occurring during parser action. According to this, for some reason an action is executed after reading the @')'@ a transition to @EXPR_ENDARG@ occurs and @'{'@ is nicely changed into @tLBRACE_ARG@ This is actually a pretty high-level technique  -  indiscriminately (ab)using _[活用:)]_ the LALR(1) up to the (1).

h5(#abusing-the-lookahead). Abusing the lookahead
...
bc(text). % ruby -yce 'm (a) {nil}' 2>&1 | egrep '^Reading|Reducing'
...
Start by looking at the middle of the list. @')'@ is read. Now look at the end  -  the reduction (execution) of embedding action (<tt>@9</tt>) finally happens. Indeed, this would allow @EXPR_ENDARG@ to be set after the @')'@ before the @'{'@ But is this always the case? Let's take another look at the part where it's set.

bc(text). Rule 1    tLPAREN_ARG  {lex_state = EXPR_ENDARG;} ')'
...
bc(text). target  : tLPAREN_ARG tmp ')'
tmp     :
            {
                lex_state = EXPR_ENDARG;
            }
...
h5(#ascertaining-lookahead). Ascertaining lookahead
...
bc(ruby). m (a        # ... tLPAREN_ARG '(' tIDENTIFIER
...
bc(ruby).                 | tLPAREN_ARG ')' { lex_state = EXPR_ENDARG; }
...
h4(#doend-iterator). @do@〜@end@ iterator
...
h4(#reality-and-truth). Reality and truth
...
bc(text). % rubylex-analyser -e  'm a,a,a,a;'
+EXPR_BEG
EXPR_BEG     C        "m"  tIDENTIFIER          EXPR_CMDARG
...
bc(text). % rubylex-analyser -e  'm a(),a,a;'
+EXPR_BEG
EXPR_BEG     C        "m"  tIDENTIFIER          EXPR_CMDARG
EXPR_CMDARG S         "a"  tIDENTIFIER          EXPR_ARG
                                              1:cmd push-
...
h5(#still-not-the-end). Still not the end
...
bc(ruby). % rubylex-analyser -e 'm m (a), a'
...
bc(text).   1:cmd push-       parserpush(1)
