...
# the text area
# a place for static variable and global variable
# the machine stack
# the heap

The text area is where the code lies. Obviously the second area holds static and global variables.
The text area is a place with the code. As you see, the second area holds static and global variables. Function calls, along with their arguments and local variables are piled up in the machine stack. The heap is allocated by `malloc()`. 
...
As for the stack frame, one corresponds to one time of the function call. When the function is called, one stack frame is piled up. When `return` is done, one stack frame is unloaded. The appearance of the machine stack is shown in Figure 1.

!images/ch_gc_macstack.jpg(Machine stack)!

In this picture the tip of the stack is above but that isn't
necessarily the case. For instance on a x86 machine the stack
extends towards the lower ranked addresses.
...
But inlike `malloc()` it's not necessary to free the memory allocated
...
But there exist environments without a native `alloca()`.
...
There is usually an object in the memory. If a lot of objects are made, a lot of memories are used as a natural result. There must actually be a limit in the amount of the memory though there is no problem if the memory can be infinitely used. Therefore, the memory not used should be collected and be recycled. It is necessary to return the memory gotten by `malloc() ` by `free() ` if it says a little more concretely.
...
GC was first implemented in Lisp. The GC in Lisp, so the world's first
GC was of the type _mark&sweep_. Ruby's GC is of the same type.
...
By the way, latest Python(2.2) uses reference count GC and the cycle can be freed. However, it is not because of the reference count, but because it occasionally uses mark and sweep to check for them. 
