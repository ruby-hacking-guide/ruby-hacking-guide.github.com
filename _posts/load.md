...
\`PATH\_SEP\` is the \`path separator\`: \`':'\` under UNIX, \`';'\` under
...
The \`rb\_load()\` which is called here is actually the "meat" of the
Ruby-level \`load\`.
...
Delegated entirely. The second argument \`script\` of \`load\_file()\` is a boolean
